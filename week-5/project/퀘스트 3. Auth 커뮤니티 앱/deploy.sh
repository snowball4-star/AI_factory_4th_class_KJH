...
rm -f "$TMP/.env.local"   # vercel link 가 만든 로컬 개발용 환경변수 파일

# 업로드 직전 마지막 확인: 임시 폴더에 어떤 .env* 도 없어야 한다
if compgen -G "$TMP/.env*" > /dev/null; then
  echo "중단: 임시 폴더에 .env 파일이 있습니다. 비밀정보가 업로드될 수 있습니다." >&2
  exit 1
fi
